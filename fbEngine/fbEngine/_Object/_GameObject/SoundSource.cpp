...
SoundSource::SoundSource(char* name):GameObject(name)
{
...
void SoundSource::InitCommon()
{
	...
	m_dspSettings.DstChannelCount = INSTANCE(SoundEngine)->GetNumChannel();
...
{
	m_waveFile = INSTANCE(SoundEngine)->GetWaveFileBank().FindWaveFile(0, filePath);
	if (!m_waveFile) {
	...
	//サウンドボイスソースを作成。
	m_sourceVoice = INSTANCE(SoundEngine)->CreateXAudio2SourceVoice(m_waveFile.get(), is3DSound);
	if (is3DSound) {
		INSTANCE(SoundEngine)->Add3DSoundSource(this);
	}
	InitCommon();
...
void SoundSource::Init(const NameKey& nameKey, bool is3DSound)
{
...
{
	if (m_is3DSound) {
		INSTANCE(SoundEngine)->Remove3DSoundSource(this);
		m_is3DSound = false;
	}
}
